...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [achievementState, setAchievementState] = useState({ unlocked: [] as string[], points: 0 });
  ...
  return <div className="flex h-[100dvh] w-full bg-background overflow-hidden text-foreground">
    ...
    {showAchievements && user && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onMouseDown={e => { if (e.target === e.currentTarget) setShowAchievements(false); }}><section className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-3xl border border-border bg-card shadow-2xl p-6"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-3"><div className="p-2.5 rounded-xl bg-yellow-400/15 text-yellow-500"><Trophy className="w-6 h-6" /></div><div><h2 className="text-2xl font-bold">Achievements</h2><p className="text-sm text-muted-foreground">Complete milestones to earn Pulse Points.</p></div></div><button onClick={() => setShowAchievements(false)} className="p-2 rounded-xl hover:bg-secondary cursor-pointer"><X className="w-5 h-5" /></button></div><div className="my-5 rounded-2xl bg-yellow-400/10 border border-yellow-400/20 p-4"><span className="text-xs text-muted-foreground">Pulse Points</span><div className="text-3xl font-black text-yellow-500">{achievementState.points}</div></div><div className="space-y-2">{ACHIEVEMENTS.map(a => { const unlocked=achievementState.unlocked.includes(a.id); return <div key={a.id} className={`flex items-center gap-3 p-3 rounded-2xl border border-border ${unlocked?"bg-yellow-400/5":"opacity-45 grayscale"}`}><div className="w-10 h-10 rounded-xl bg-yellow-400/15 text-yellow-500 grid place-items-center text-lg">{a.icon}</div><div className="flex-1 min-w-0"><div className="font-semibold">{a.name}{unlocked && <span className="ml-2 text-[10px] uppercase tracking-wide text-green-500">Unlocked</span>}</div><p className="text-xs text-muted-foreground">{a.description} · +{a.points} points</p></div></div>; })}</div><button onClick={() => setShowAchievements(false)} className="w-full mt-5 h-11 rounded-xl bg-primary text-primary-foreground font-semibold hover:opacity-90 cursor-pointer">Done</button></section></div>}
  </div>;
}
