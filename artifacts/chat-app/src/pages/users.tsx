...
export default function Users() {
  ...
  const { data: users, isLoading } = useListUsers(
    { search: searchParam },
    { query: { enabled: true, queryKey: getListUsersQueryKey({ search: searchParam }) } }
  );
  ...
  return (
    <div className="flex flex-col w-full h-full max-w-4xl mx-auto p-4 md:p-8">
      <div className="flex items-center gap-3 mb-8">
        <div className="bg-primary/10 text-primary p-3 rounded-2xl"><SearchIcon className="w-6 h-6" /></div>
        <div><h1 className="text-3xl font-bold text-foreground tracking-tight">Directory</h1><p className="text-muted-foreground">Find people to chat with on Pulse</p></div>
      </div>
      ...
      <div className="relative mb-4 group">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name or username..." className="pl-12 h-14 text-lg bg-card/50 border-border hover:border-primary/50 transition-colors rounded-2xl shadow-sm" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground"><div className="w-20 h-20 bg-secondary rounded-full flex items-center justify-center mb-6"><SearchIcon className="w-8 h-8 opacity-50" /></div><p className="text-lg">No users found matching "{search}"</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {users.map((user: User) => (
              <div key={user.id} className="flex items-center justify-between p-4 bg-card border border-border rounded-2xl hover:border-primary/50 hover:bg-secondary/20 transition-all group">
