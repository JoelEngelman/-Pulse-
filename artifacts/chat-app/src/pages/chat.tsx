...
import { ChevronLeft, Send, MoreVertical, Loader2, Phone } from "lucide-react";
...
const PRESENCE_API = "https://pulse-api.joeldavidengelman.workers.dev";

function isActuallyOnline(user: any) {
  if (!user) return false;
  const lastSeen = user.lastSeen ? new Date(user.lastSeen).getTime() : 0;
  return !!user.isOnline && (!lastSeen || Date.now() - lastSeen < 45_000);
}
...
export default function Chat() {
  ...
  useEffect(() => {
    let alive = true;
    const beat = () => { if (alive) fetch(`${PRESENCE_API}/api/users/me/heartbeat`, { method: "POST", credentials: "include" }).catch(() => {}); };
    beat();
  ...
  useEffect(() => { if (conversationId && conversation?.unreadCount) markRead.mutate({ conversationId: hookConversationId }, { onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/conversations"] }) }); }, [conversationId, conversation?.unreadCount]);
  ...
  const awardMessageAchievement = () => { if (currentUser?.username) recordMessage(currentUser.username); };
  const handleSend = (e: React.FormEvent) => {
    ...
    sendMessage.mutate({ conversationId: hookConversationId, data: { content: trimmed } }, {
      onSuccess: () => { setContent(""); awardMessageAchievement(); queryClient.invalidateQueries({ queryKey: getGetMessagesQueryKey(hookConversationId) }); queryClient.invalidateQueries({ queryKey: ["/api/conversations"] }); setTyping.mutate({ conversationId: hookConversationId, data: { isTyping: false } }); },
      onError: (error: any) => setSendError(error?.message || "Couldn't send that message. Please try again.")
    });
  };

  const handleMediaSelect = (mediaContent: string) => {
    setSendError("");
    sendMessage.mutate({ conversationId: hookConversationId, data: { content: mediaContent } }, { onSuccess: () => { awardMessageAchievement(); queryClient.invalidateQueries({ queryKey: getGetMessagesQueryKey(hookConversationId) }); queryClient.invalidateQueries({ queryKey: ["/api/conversations"] }); }, onError: (error: any) => setSendError(error?.message || "Couldn't send that message. Please try again.") });
  };
  ...
  const online = isActuallyOnline(otherUser);
