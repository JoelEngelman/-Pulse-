...
export default function Chat() {
  ...
  return <div className="flex w-full h-full">
    ...
    <div className="flex-1 flex flex-col bg-background/50 h-full relative">
      ...
      <div className="p-4 bg-card/30 border-t border-border flex-shrink-0">{sendError && <div className="mb-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">{sendError}</div>}<form onSubmit={handleSend} className="flex items-center gap-2 bg-background border border-border rounded-xl pr-2 focus-within:ring-1 focus-within:ring-primary focus-within:border-primary transition-all overflow-hidden"><div className="pl-2 flex-shrink-0"><MediaButton onSelect={handleMediaSelect} /></div><Input value={content} onChange={(e) => setContent(e.target.value)} placeholder="Type a message..." className="flex-1 border-0 bg-transparent ring-0 focus-visible:ring-0 focus-visible:ring-offset-0 px-2 h-12" /><Button type="submit" size="icon" variant="glow" disabled={!content.trim() || sendMessage.isPending} className="h-9 w-9 rounded-lg shrink-0 cursor-pointer disabled:cursor-not-allowed"><Send className="w-4 h-4 ml-0.5" /></Button></form></div>
    </div>
  </div>;
}
