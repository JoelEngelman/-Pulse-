...
function makeDisplacementMap(w: number, h: number, radius: number) {
  ...
  const bezel = Math.max(10, Math.min(cw, ch) * 0.075);
  const r = Math.max(2, radius * scale);

  for (let y = 0; y < ch; y++) {
    for (let x = 0; x < cw; x++) {
      const edgeX = Math.min(x, cw - 1 - x);
      const edgeY = Math.min(y, ch - 1 - y);
      const dx = Math.max(0, r - edgeX);
      const dy = Math.max(0, r - edgeY);
      const d = Math.min(edgeX, edgeY);
      const t = Math.max(0, Math.min(1, d / bezel));
      const strength = (1 - t) * (1 - t) * 0.9;
      const nx = x < cw / 2 ? 1 : -1;
      const ny = y < ch / 2 ? 1 : -1;
      const i = (y * cw + x) * 4;
      data[i] = 128 + nx * strength * 110;
      data[i + 1] = 128 + ny * strength * 110;
      data[i + 2] = 128;
      data[i + 3] = 255;
    }
  }
...
function applyLiquidGlass(el: HTMLElement): GlassInstance {
  ...
  const refresh = () => {
    ...
    image.setAttribute("href", makeDisplacementMap(rect.width, rect.height, radius));
  };
  ...
  return {
    ...
    destroy: () => {
      observer.disconnect();
      filter.remove();
      el.classList.remove("pulse-liquid-glass");
      el.style.removeProperty("--pulse-liquid-filter");
...
function shouldGlass(el: Element) {
  ...
  if (el.dataset.liquidGlass === "off") return false;
  if (el.classList.contains("pulse-liquid-glass")) return false;
  if (el.classList.contains("glass-panel")) return true;
  const cls = el.className;
  if (typeof cls !== "string") return false;
  // Apply to the major UI surfaces, not tiny icon/background elements.
  if (!/(bg-card\\/(30|40|50)|bg-background\\/50|bg-secondary\\/(20|30|35|40|50))/.test(cls)) return false;
  const r = el.getBoundingClientRect();
  return r.width >= 180 && r.height >= 44 && r.width * r.height < 700000;
}
...
function scan(root: ParentNode = document) {
  ...
  for (const el of candidates) {
    if (!shouldGlass(el)) continue;
    instances.set(el, applyLiquidGlass(el));
  }
}
    ...
    for (const mutation of mutations) {
      for (const node of Array.from(mutation.addedNodes)) {
        if (node instanceof HTMLElement) {
          if (shouldGlass(node)) instances.set(node, applyLiquidGlass(node));
          scan(node);
        }
      }
    }
